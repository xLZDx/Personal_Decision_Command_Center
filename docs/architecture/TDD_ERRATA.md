...
details wrong needs an explicit, reviewable supersession mechanism.

**Nothing in this file changes an invariant.** INV-01..INV-31 (TDD §5) may not be amended here.
Invariant changes require ADR + independent review + operator approval and are recorded separately
in `docs/architecture/TDD_INVARIANT_AMENDMENTS.md`.

**Architecture reading order once an invariant amendment is adopted:**
...
A future agent MUST NOT resurrect superseded invariant wording merely because it still exists in
the frozen TDD or a historical review.
...
If either later turns out to change an invariant rather than a figure, it belongs in
`TDD_INVARIANT_AMENDMENTS.md`, not this file.
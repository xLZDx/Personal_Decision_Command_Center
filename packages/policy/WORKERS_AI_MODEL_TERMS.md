...
- Callable model: `@cf/meta/llama-3.1-8b-instruct-fast`.
- Cloudflare describes it as Cloudflare-hosted, with a 128,000-token context window, and lists it
  among JSON Mode models.
...
Cloudflare links this model to Meta's Llama 3.1 terms. Meta publishes the Llama 3.1 Community License
...
Cloudflare documents a free allocation of 10,000 Neurons/day, reset at 00:00 UTC. The callable
`...instruct-fast` alias and the pricing-table `...instruct-fp8-fast` label are not identical. Until
Cloudflare documents that they are the same billing identity, MVP1 does not rely on the lower fast
rate. Reservation and reconciliation use the higher published non-fast Llama 3.1 8B rates:

- 25,608 Neurons per million input tokens;
- 75,147 Neurons per million output tokens.
...
The maximum permitted request therefore reserves 482 Neurons. When complete token usage is returned,
...
- Cloudflare model page: https://developers.cloudflare.com/workers-ai/models/llama-3.1-8b-instruct-fast/
